# ga_bot_4.3.py
# Добавил 3-ий скрытый слой в MLP
# =========================================================
# Genetic MLP Bot
# =========================================================
# Features:
...
BEST_GENOME_FILE = "ga_bot_4.3_best_genome.npy"
CHECKPOINT_FILE = "ga_bot_4.3_population.pkl"
...
class Genome:
    ...
    def __init__(
        ...
        if weights is not None:
            ...
            # Xavier init
            limit1 = np.sqrt(
                6.0 / (in_dim + hidden_dim_1)
            )

            limit2 = np.sqrt(
                6.0 / (hidden_dim_1 + 1 + hidden_dim_2)
            )

            limit3 = np.sqrt(
                6.0 / (hidden_dim_2 + 1 + hidden_dim_3)
            )

            limit4 = np.sqrt(
                6.0 / (hidden_dim_3 + 1 + out_dim)
            )
            ...
            self.w2 = np.random.uniform(
                -limit2,
                limit2,
                (hidden_dim_1 + 1, hidden_dim_2)
            )

            self.w3 = np.random.uniform(
                -limit3,
                limit3,
                (hidden_dim_2 + 1, hidden_dim_3)
            )

            self.w4 = np.random.uniform(
                -limit4,
                limit4,
                (hidden_dim_3 + 1, out_dim)
            )
    ...
    def forward(self, observation):
        ...
        hidden3 = np.tanh(hidden3)

        hidden3 = np.concatenate(
            [hidden3, [1.0]]
        )
    ...
    def mutate(
        ...
        self.w3 += (
            mask3
            *
            np.random.randn(*self.w3.shape)
            *
            strength
        )

        mask4 = (
            np.random.rand(*self.w4.shape)
            < rate
        )
