...
class RuntimeEvaluator:
    ...
    def build_state(self, data, angle):
        ...
        additional = [min(max(v, 0.0), 1.0) for v in additional_info.values()]
...
async def async_main():
    ...
    while True:
        ...
        # Ищу конфликт
        for gid, genome in population.population.items():

            ids = set(genome.nodes.keys())
