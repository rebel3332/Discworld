...
# Добавил обновленную (относительную) механику управления 
...
class Genome:
    ...
    def forward(self, observation):
        ...
        inputs = np.array(
            [min(max(x, 0.0), 1.0) for x in w] +
            [min(max(x, 0.0), 1.0) for x in e] +
            [1.0]  # bias
        )
        ...
        return {
            "move_front_back": move_front_back,
...
class Population:
    ...
    def report_life_end(self, genome, fitness, duration):
        ...
        if fitness > self.best_ever:
            ...
            genome.save("ga_bot_3_best_genome.npy")
...
async def main():
    ...
    try:
        pop.individuals[0] = Genome.load("ga_bot_3_best_genome.npy")
