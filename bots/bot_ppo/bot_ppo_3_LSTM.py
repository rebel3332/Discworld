...
class PPOBot:
    ...
    async def process_state(self, ws, state):
        ...
        # # телепорт обнаруживается по резкому изменению координат, 
        # # обнуляем hidden state чтобы не портить обучение
        # teleport_dist = math.hypot(
        #     me["x"] - self.prev_x,
        #     me["y"] - self.prev_y
        # )

        # if teleport_dist > 200:
        #     self.hidden = None
    ...
    def select_action(self, obs):
        ...
        with torch.no_grad():
            logits, value, self.hidden = self.model(
                state,
                self.hidden
            )
    ...
    def train(self):
        ...
        logits, values, _ = self.model(states, None)
